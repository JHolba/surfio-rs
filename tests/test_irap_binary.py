...
def test_short_files_result_in_value_error():
    ...
    file_buffer = surfio.write_irap_binary_buffer(srf)
    truncated_buffer = file_buffer[:100]
    with pytest.raises(ValueError, match="fill|end of file|buffer"):
        _ = surfio.read_irap_binary_buffer(truncated_buffer)
...
def test_surfio_can_import_data_exported_from_surfio():
    ...
    buffer = surfio.write_irap_binary_buffer(srf)
    srf_imported = surfio.read_irap_binary_buffer(buffer)
...
def test_xtgeo_can_import_data_exported_from_surfio(tmp_path):
    ...
    surfio.write_irap_binary_file(str(tmp_path / "test.irap"), srf)
...
def test_exporting_nan() -> None:
    ...
    srf_export = surfio.write_irap_binary_buffer(surface)
